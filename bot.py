...
if __name__ == "__main__":
    # ВАРИАНТ 1: Только Flask (без бота) - для Render Web Service
    # Запускаем только веб-сервер для порта
    port = int(os.environ.get("PORT", 10000))
    print(f"🌐 Веб-сервер запущен на порту {port}")
    print("⚠️ Бот отключен для избежания конфликта")
    app.run(host='0.0.0.0', port=port)
    
    # ВАРИАНТ 2: Только бот (без Flask) - для Render Background Worker
    # Раскомментируйте следующую строку и закомментируйте выше, если используете Background Worker
    # run_bot()